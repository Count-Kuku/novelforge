...
<template>
  <div class="planned-shell">
    ...
    <main class="planned-main">
      <header class="planned-topbar"><div><p class="eyebrow"><span class="crumb-project">{{ workspace.activeProject?.title || workspace.activeProject?.name || '项目' }}</span><span class="crumb-sep">▸</span>当前故事</p><div class="title-line"><select v-if="workspace.activeStories.length" class="story-switcher" :value="workspace.activeStoryId" aria-label="选择故事" @change="changeStory"><option v-for="story in workspace.activeStories" :key="story.story_id" :value="story.story_id">{{ story.name }}</option></select><span v-else class="story-empty">暂无进行中的故事</span></div></div><div class="top-actions"><button class="mode-toggle" @click="switchToConversation">切到对话工作台</button><span class="pill">规划模式</span><span class="avatar" aria-hidden="true">NF</span></div></header>
      <div v-if="workspace.activeProjectId && !workspace.activeStories.length && route.name !== 'planned-direction'" class="planned-new-story"><span>当前项目还没有进行中的故事。</span><RouterLink to="/planned/direction">前往创建第一个规划故事 →</RouterLink></div>

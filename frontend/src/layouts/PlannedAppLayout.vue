...
async function loadStructure() {
  ...
  try { structure.value = await api.structure(workspace.activeProjectId, workspace.activeStory.story_id) } catch { structure.value = { volumes: [], arcs: [], chapters: [] } }
}
...
async function renameCurrentStory() {
  ...
  const name = globalThis.prompt('新的故事名称', workspace.activeStory.name)
...
async function renameCurrentProject() {
  ...
  const name = globalThis.prompt('新的项目名称', workspace.activeProject.name)
...
async function archiveCurrentStory() {
  if (!workspace.activeProjectId || !workspace.activeStory) return
  if (!globalThis.confirm(`归档“${workspace.activeStory.name}”？归档后仍可从数据库恢复。`)) return
  ...
  <div class="planned-shell">
    <aside class="planned-rail">
      ...
      <nav class="planned-nav" aria-label="规划导航">
        <p class="eyebrow">BUILD THE NOVEL</p>
    ...
    <main class="planned-main">
      <header class="planned-topbar"><div><p class="eyebrow">LONG-FORM MODE</p><div class="title-line"><select class="story-switcher" :value="workspace.activeStoryId" aria-label="选择故事" @change="changeStory"><option v-for="story in workspace.stories" :key="story.story_id" :value="story.story_id">{{ story.name }}</option></select></div></div><div class="top-actions"><button class="mode-toggle" @click="switchToConversation">切到对话工作台</button><span class="pill">规划模式</span><button class="avatar">NF</button></div></header>
      <section v-if="workspace.error" class="connection-banner">{{ workspace.error }}<span>请确认 API 服务已启动。</span></section>
      <RouterView />
    </main>
  </div>
</template>

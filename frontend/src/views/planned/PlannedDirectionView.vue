...
<template>
  <section class="planned-page">
    ...
    <div v-if="!hasStory && !workspace.activeProjectId" class="empty-panel"><span class="empty-icon">✦</span><h3>还没有故事</h3><p>请先返回入口创建项目和第一个故事。</p><RouterLink to="/" class="button accent">返回入口</RouterLink></div>
    <div v-else-if="!hasStory" class="empty-panel"><span class="empty-icon">✦</span><h3>开始第一个规划故事</h3><p>为当前项目创建一个规划故事，就可以推进创作方向、结构与章节。</p><div class="empty-inline"><NewStoryInline :default-mode="'planned'" /></div></div>
...
.empty-panel { padding: 72px 30px; text-align: center; }.empty-icon { display: block; margin-bottom: 15px; color: var(--accent); font-size: 31px; }.empty-panel h3 { margin: 0; font-family: Georgia, serif; font-size: 25px; font-weight: 400; }.empty-panel p { color: var(--muted); font-size: 13px; }.empty-inline { display: grid; justify-items: center; max-width: 460px; margin: 26px auto 0; }

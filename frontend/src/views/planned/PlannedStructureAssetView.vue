...
async function approveDiscussion() {
  ...
  try { await api.approveDiscussion(workspace.activeProjectId, workspace.activeStory.story_id, assetType.value, discussionStep.value, assetNo.value); discussionText.value = '讨论已批准并写入当前结构资产。' } catch (reason) { discussionText.value = reason instanceof ApiClientError ? reason.message : '批准失败' } finally { approving.value = false }
}

async function deleteAsset() {
  if (!workspace.activeProjectId || !workspace.activeStory || deleting.value) return
  if (!globalThis.confirm(`确认删除${assetType.value === 'volume' ? '分卷' : '剧情段'} ${assetNo.value}？该操作会级联移除其结构资产。`)) return
  deleting.value = true
  try {
...
<template>
  <button class="danger-action" :disabled="deleting" @click="deleteAsset">{{ deleting ? '删除中…' : '删除此结构资产' }}</button>
  <article v-if="assetType === 'arc' && !loading" class="chapter-plan-panel"><p class="eyebrow">CHAPTER PLAN</p><h3>章节分配与冲突检查</h3><p class="plan-hint">保存前会检查重复章节、剧情段之间的重叠、预计章节数和现有结构归属；冲突必须人工合并后才能写入。</p><textarea v-model="chapterPlanJson" rows="8" aria-label="章节计划 JSON"></textarea><textarea v-model="chapterPlanReport" rows="4" placeholder="冲突或人工裁决说明"></textarea><button class="button secondary" :disabled="chapterPlanSaving" @click="saveChapterPlan">{{ chapterPlanSaving ? '校验中…' : '校验并保存章节计划' }}</button><div v-if="planValidation" class="validation-box" :class="{ invalid: !planValidation.valid }"><strong>{{ planValidation.valid ? '校验通过' : '发现结构冲突' }}</strong><span v-for="item in (planValidation.conflicts || planValidation.warnings || [])" :key="`${item.code}-${item.chapter_no || item.index}`">{{ item.message }}</span></div></article>
  <section class="asset-page"><p class="eyebrow">{{ assetType === 'volume' ? 'VOLUME WORKSPACE' : 'ARC WORKSPACE' }}</p><div class="asset-heading"><div><h2>{{ assetType === 'volume' ? `第 ${assetNo} 卷` : `剧情段 ${assetNo}` }}<em>的推进。</em></h2><p>{{ metadata.title || metadata.name || '未命名结构资产' }} · 讨论、细纲和章节分配在同一处保存。</p></div><span class="pill">{{ saved ? '已保存' : '规划资产' }}</span></div><div v-if="loading" class="asset-state">正在读取结构资产…</div><div v-else class="asset-grid"><article class="asset-editor"><div class="editor-head"><div><p class="eyebrow">CANONICAL OUTLINE</p><h3>{{ assetType === 'volume' ? '分卷大纲' : '剧情段大纲' }}</h3></div><button class="button accent" :disabled="saving" @click="save">{{ saving ? '保存中…' : '保存大纲' }}</button></div><textarea v-model="outline" rows="18" placeholder="把阶段目标、冲突推进和章节边界写在这里…"></textarea><p v-if="error" class="asset-error">{{ error }}</p><article class="asset-discussion"><p class="eyebrow">ASSET DISCUSSION</p><h3>先讨论取舍，再保存结构。</h3><div class="discussion-row"><input v-model="discussionIdea" placeholder="想讨论这一卷/剧情段的目标、冲突或章节边界？" @keydown.enter="discuss" /><button class="button secondary" :disabled="discussing || !discussionIdea.trim()" @click="discuss">{{ discussing ? '讨论中…' : '开始讨论' }}</button></div><div v-if="discussionText" class="discussion-result"><p>{{ discussionText }}<span v-if="discussing">▌</span></p><button v-if="discussionStep" class="button accent" :disabled="approving" @click="approveDiscussion">{{ approving ? '批准中…' : '批准并应用' }}</button></div></article></article><aside class="asset-inspector"><p class="eyebrow">ASSET INSPECTOR</p><h3>当前状态</h3><dl><div v-for="(value, key) in metadata" :key="key"><dt>{{ key }}</dt><dd>{{ typeof value === 'object' ? JSON.stringify(value) : value }}</dd></div></dl><div class="approval-card"><strong>{{ discussion.approval_ready ? '讨论已达到批准条件' : '尚无已批准讨论' }}</strong><small>批准结论会成为下一阶段生成的上下文。</small></div></aside></div></section>
</template>

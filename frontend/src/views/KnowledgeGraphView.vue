...
<template>
  <section class="graph-page"><div class="graph-heading"><div><p class="eyebrow">KNOWLEDGE GRAPH</p><h1>让设定之间的关系，<em>被看见。</em></h1><p>这里只投影正式知识和关系，不直接修改检索投影；编辑仍通过知识工作区提交。</p></div><button class="button secondary" :disabled="loading" @click="loadGraph">{{ loading ? '读取中…' : '刷新关系图' }}</button></div><div v-if="loading" class="graph-state">正在读取正式关系…</div><p v-else-if="error" class="graph-error">{{ error }}</p><div v-else class="graph-grid"><article><p class="eyebrow">NODES · {{ graph.nodes?.length || 0 }}</p><div v-if="!graph.nodes?.length" class="muted">当前故事还没有关系节点。</div><div v-for="node in graph.nodes || []" :key="node.node_id || node.id" class="graph-node"><strong>{{ node.label || node.name || node.node_id }}</strong><small>{{ node.node_type || node.type || 'entity' }}</small></div></article><article><p class="eyebrow">EDGES · {{ graph.edges?.length || 0 }}</p><div v-if="!graph.edges?.length" class="muted">当前故事还没有关系边。</div><div v-for="edge in graph.edges || []" :key="edge.edge_id || `${edge.source_id}-${edge.target_id}`" class="graph-edge"><span>{{ edge.source_label || edge.source_id }}</span><b>{{ edge.relation_type || edge.label || '关联' }}</b><span>{{ edge.target_label || edge.target_id }}</span></div></article></div></section>
</template>

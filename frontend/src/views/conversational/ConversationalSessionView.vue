...
async function extractFragment(fragmentId: string) {
  ...
  try {
    ...
    const confirmedCount = Array.isArray(autoConfirm.confirmed_ids) ? autoConfirm.confirmed_ids.length : 0
    const blockedCount = Array.isArray(autoConfirm.blocked_ids) ? autoConfirm.blocked_ids.length : 0
    ...
    else if (confirmedCount && blockedCount) extractNotice.value = `已提炼 ${queuedCount || (confirmedCount + blockedCount)} 条设定：${confirmedCount} 条已确认，${blockedCount} 条待审核。`
    else if (blockedCount) extractNotice.value = `已提炼 ${blockedCount} 条候选，需人工审核后进入正式知识。`
...
.extract-notice { margin: 12px 0; padding: 9px 12px; border: 1px solid rgba(222,172,139,.3); border-radius: 9px; color: #d8c2ae; background: rgba(190,111,78,.1); font-size: 11px; }

...
async function reloadBundle() {
  ...
  try { actions.value = (await api.actions(workspace.activeProjectId, workspace.activeStory.story_id, String(route.params.sessionId))).actions } catch { actions.value = [] }
}
...
async function renameSession() {
  if (!workspace.activeProjectId || !workspace.activeStory || !route.params.sessionId) return
  const title = globalThis.prompt('会话名称', sessionTitle.value || '未命名会话')
...
<template>
  ...
  <aside v-if="fragments.length" class="fragment-timeline"><strong>版本时间线</strong><span v-for="(fragment, index) in fragments" :key="`timeline-${fragment.fragment_id}`"><i :class="{ active: fragment.status === 'accepted' }"></i>V{{ index + 1 }} · {{ fragment.status }} · {{ fragment.created_at }}</span></aside>
  <section class="session-page"><div class="session-title"><p class="eyebrow">CONTINUING THE THREAD</p><h1>{{ sessionTitle || workspace.activeStory?.name }}</h1><span class="pill">会话 {{ String(route.params.sessionId).slice(-8) }}</span><button class="session-rename" @click="renameSession">重命名</button></div><div v-if="loading" class="session-state">正在找回这段对话…</div><div v-else-if="error && !turns.length" class="session-state error">{{ error }}</div><div v-else class="session-content"><div v-if="actions.length" class="action-list"><p class="eyebrow">ACTION CARDS</p><article v-for="action in actions" :key="action.action_id" class="action-card"><div><strong>{{ action.action_type }}</strong><span class="action-status">{{ action.status }}</span><p>{{ action.plan?.message || action.error_text || '待处理的创作动作' }}</p></div><div class="action-buttons"><button v-if="action.status === 'awaiting_confirmation' || action.status === 'planned'" class="button secondary" @click="executeAction(action)">确认执行</button><button v-if="action.status === 'awaiting_confirmation'" class="button ghost" @click="cancelAction(action)">取消</button><button v-if="action.status === 'completed' && action.result" class="button ghost" @click="undoAction(action)">撤销</button></div></article></div><div v-if="!turns.length && !fragments.length" class="session-state">这是一段刚刚打开的空间。下一句话，从你想探索的地方开始。</div><div v-for="turn in turns" :key="turn.turn_id" class="turn-row"><span class="turn-dot"></span><div><small>你 · {{ turn.created_at }}</small><p>{{ turn.user_message }}</p></div></div><div v-for="fragment in fragments" :key="fragment.fragment_id" class="fragment-card"><small>NovelForge · {{ fragment.status }}</small><p>{{ fragment.content }}</p><div v-if="fragment.status === 'proposed'" class="fragment-actions"><button class="button secondary" @click="accept(fragment.fragment_id)">接受片段</button><button class="button ghost" @click="actionType = 'rewrite'; draft = '请重写这个片段';">重写</button><button class="button ghost" @click="actionType = 'branch'; draft = '从这个版本继续';">分支</button><span class="muted">候选版本</span></div></div><div v-if="streamingText" class="fragment-card streaming"><small>NovelForge · 正在生成</small><p>{{ streamingText }}<span class="cursor">▌</span></p></div><div v-if="attachments.length" class="attachment-list"><span v-for="attachment in attachments" :key="attachment.attachment_id">📎 {{ attachment.title || attachment.filename }}</span></div></div><p v-if="error" class="inline-error">{{ error }}</p><div v-if="attachmentOpen" class="attachment-tray"><input v-model="attachmentTitle" placeholder="资料标题" /><textarea v-model="attachmentText" rows="4" placeholder="粘贴一段资料，只在当前会话中使用…"></textarea><input v-model="attachmentUrl" type="url" placeholder="或输入网页 URL（仅抓取公开页面）" /><input ref="attachmentFile" type="file" accept=".txt,.md,.pdf,.docx,.epub" multiple @change="addFileAttachment" /><small v-if="batchUploadMessage" class="batch-upload-message">{{ batchUploadMessage }}</small><div><button class="button secondary" :disabled="attachmentSaving || !attachmentText.trim()" @click="addAttachment">{{ attachmentSaving ? '保存中…' : '添加资料' }}</button><button class="button secondary" :disabled="attachmentSaving || !attachmentUrl.trim()" @click="addUrlAttachment">{{ attachmentSaving ? '抓取中…' : '添加网页' }}</button><button class="button ghost" @click="attachmentOpen = false">取消</button></div></div><div class="session-composer"><select v-model="actionType" aria-label="生成动作"><option value="continue">继续写作</option><option value="rewrite">重写当前方向</option><option value="branch">从候选版本分支</option></select><textarea v-model="draft" rows="2" placeholder="继续说下去……" :disabled="sending" @compositionstart="composing = true" @compositionend="composing = false" @keydown.meta.enter.prevent="!composing && send()" @keydown.ctrl.enter.prevent="!composing && send()"></textarea><button class="attach-button" title="添加资料" @click="attachmentOpen = !attachmentOpen">＋资料</button><button class="attach-button" title="规划动作" @click="planActionFromDraft">＋动作</button><button class="button accent" :disabled="sending || !draft.trim()" @click="send">{{ sending ? '生成中…' : '发送' }}</button></div></section>
</template>

...
<template>
  <section class="conversation-home"><div class="welcome"><p class="eyebrow">A SMALL DOOR INTO THE STORY</p><h1>现在脑子里，<br /><em>最想说的是什么？</em></h1><p>不用整理，不用先做决定。把一个画面、一句对白，或者一个还说不清楚的感觉交给我。</p></div><div class="composer-card"><textarea v-model="idea" rows="4" placeholder="比如：我想写一个人，在多年后回到一座已经不记得他的城市……" @keydown.meta.enter.prevent="begin" @keydown.ctrl.enter.prevent="begin"></textarea><div class="composer-footer"><span>⌘ / Ctrl + Enter 开始</span><button class="button accent" :disabled="creating || !idea.trim()" @click="begin">{{ creating ? '正在打开…' : '开始这段对话  →' }}</button></div></div><p v-if="error" class="error">{{ error }}</p><div class="promise-row"><div><span>✦</span><strong>轻一点</strong><small>只在你需要时出现设置</small></div><div><span>⌁</span><strong>记得住</strong><small>重要信息会被提炼保存</small></div><div><span>◌</span><strong>随时回来</strong><small>每个想法都有它的位置</small></div></div></section>
</template>

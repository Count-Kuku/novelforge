...
<template>
  <section class="conversation-home"><div class="welcome"><p class="eyebrow">{{ workspace.activeStory ? '新建会话' : '开始创作' }}</p><h1 v-if="workspace.activeStory">这次要讨论或<em>写什么？</em></h1><h1 v-else>先创建一个<em>新故事</em></h1><p>{{ workspace.activeStory ? '输入一个写作目标、待解决的问题，或直接贴入要续写和修改的内容。' : '对话工作台围绕“故事”展开。为当前项目新建一个故事后，就能开始写作或讨论。' }}</p></div><div v-if="workspace.activeStory" class="context-row"><span>正在写入</span><strong>{{ workspace.activeProject?.title || workspace.activeProject?.name }}</strong><i>▸</i><b>{{ workspace.activeStory.name }}</b><button class="rename-story" title="重命名故事" aria-label="重命名故事" @click="renameStory">✎</button><small>{{ workspace.activeStory.creation_mode === 'conversational' ? '对话故事' : '规划故事' }}</small></div><div v-if="workspace.activeStory" class="composer-card"><textarea v-model="idea" rows="4" aria-label="会话目标" placeholder="例如：试写主角多年后回到故乡的开场场景" @keydown.meta.enter.prevent="begin" @keydown.ctrl.enter.prevent="begin"></textarea><div class="composer-footer"><span>⌘ / Ctrl + Enter 创建会话</span><button class="button accent" :disabled="creating || !idea.trim()" @click="begin">{{ creating ? '正在创建…' : '创建会话  →' }}</button></div></div><div v-else class="composer-card new-story-card"><NewStoryInline :default-mode="'conversational'" /></div><p v-if="error" class="error">{{ error }}</p><div class="promise-row"><div><span>✦</span><strong>直接开始</strong><small>先创建会话，再按需调整设置</small></div><div><span>⌁</span><strong>保存版本</strong><small>写作片段和采用状态可追溯</small></div><div><span>◌</span><strong>保留上下文</strong><small>附件与确认知识可继续使用</small></div></div></section>
</template>

<style scoped>
.conversation-home { max-width: 780px; margin: clamp(40px, 10vh, 105px) auto 0; padding: 0 26px 70px; }.welcome h1 { margin: 10px 0 21px; color: #f0ebe4; font-family: Georgia, serif; font-size: clamp(42px, 6.5vw, 72px); font-weight: 400; letter-spacing: -.06em; line-height: 1.05; }.welcome h1 em { color: #e0a17d; font-style: normal; }.welcome > p:not(.eyebrow) { max-width: 500px; margin: 0; color: #9a9e96; font-size: 14px; line-height: 1.8; }.context-row { display: flex; align-items: center; gap: 8px; margin-top: 30px; color: #9a9e96; font-size: 12px; }.context-row span { color: #a4a99f; letter-spacing: .06em; }.context-row strong { color: #d6b49c; font-weight: 500; }.context-row i { color: #6f756d; font-style: normal; }.context-row b { color: #f0ebe4; font-family: Georgia, serif; font-size: 15px; font-weight: 400; }.context-row small { padding: 2px 8px; border: 1px solid rgba(255,255,255,.12); border-radius: 99px; color: #a4a99f; font-size: 10px; }.rename-story { padding: 2px 6px; border: 0; border-radius: 6px; color: #7f857c; background: transparent; cursor: pointer; font-size: 12px; line-height: 1; }.rename-story:hover { color: #f0d8c4; background: rgba(255,255,255,.07); }.composer-card { margin-top: 42px; padding: 18px; border: 1px solid rgba(255,255,255,.14); border-radius: 18px; background: rgba(55,58,55,.72); box-shadow: 0 20px 55px rgba(0,0,0,.12); }.composer-card textarea { display: block; width: 100%; resize: vertical; border: 0; outline: 0; color: #eee9e1; background: transparent; font-family: Georgia, serif; font-size: 18px; line-height: 1.7; }.composer-card textarea::placeholder { color: #a4a99f; }.composer-footer { display: flex; align-items: center; justify-content: space-between; margin-top: 12px; padding-top: 14px; border-top: 1px solid rgba(255,255,255,.09); }.composer-footer span { color: #a4a99f; font-size: 11px; }.button:disabled { cursor: not-allowed; opacity: .45; }.error { color: #df9e86; font-size: 13px; }.new-story-card { min-width: 0; }.promise-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 18px; margin-top: 64px; padding-top: 25px; border-top: 1px solid rgba(255,255,255,.09); }.promise-row div { display: grid; grid-template-columns: 23px 1fr; align-items: baseline; column-gap: 7px; }.promise-row span { color: #db9775; }.promise-row strong { color: #cbc6bc; font-family: Georgia, serif; font-size: 14px; font-weight: 400; }.promise-row small { grid-column: 2; margin-top: 5px; color: #a4a99f; font-size: 11px; }

...
async function load() {
  ...
  try {
    record.value = await api.knowledgeDetail(workspace.activeProjectId, type, id, workspace.activeStory?.story_id, workspace.activeBranchId || undefined)
    const resolvedId = String(record.value.knowledge_id || record.value.record_id || id)
    if (resolvedId && resolvedId !== id && workspace.activeStory?.story_id) {
      await router.replace({ name: route.name || undefined, params: { ...route.params, recordId: resolvedId } })
    }
    ...
    revisions.value = (await api.knowledgeRevisions(workspace.activeProjectId, type, resolvedId, workspace.activeStory?.story_id, workspace.activeBranchId || undefined)).revisions || []
    ...
    evidence.value = (await api.knowledgeEvidence(workspace.activeProjectId, type, resolvedId, workspace.activeStory?.story_id, workspace.activeBranchId || undefined)).evidence || []
    await nextTick()
    hydrated.value = true
...
async function save() {
  ...
  try {
    const currentId = String(record.value.knowledge_id || record.value.record_id || route.params.recordId || '')
    const result = await api.updateKnowledge(workspace.activeProjectId, String(route.params.recordType || 'knowledge'), currentId, patch, 'Vue 类型化编辑', revisionId.value || undefined, workspace.activeStory?.story_id, workspace.activeBranchId || undefined)
    const savedId = String(result.record?.knowledge_id || result.record?.record_id || currentId)
    if (savedId && savedId !== String(route.params.recordId || '')) await router.replace({ name: route.name || undefined, params: { ...route.params, recordId: savedId } })
    record.value = result.record
    clearEditorDirty(editorId)
    message.value = result.created_override ? '已创建当前世界线独立副本并保存修订' : '类型化知识修订已保存'
    await load()
  } catch (reason) { message.value = reason instanceof ApiClientError && reason.status === 409 ? '条目已被修改，请重新加载后合并。' : reason instanceof Error ? reason.message : '保存失败' } finally { saving.value = false }
}

onMounted(load)
watch(() => [workspace.activeStoryId, workspace.activeBranchId], () => { void load() })
onUnmounted(() => clearEditorDirty(editorId))
</script>

<template>
  <section class="knowledge-editor-page"><p class="eyebrow">知识编辑</p><h1>编辑正式<em>知识条目</em></h1><p v-if="record.visible_from_checkpoint" class="override-notice">这条资料来自父世界线的冻结检查点。保存后会创建当前世界线的独立副本，父线内容保持不变。</p><div v-if="loading" class="editor-state">正在读取知识条目…</div><template v-else><div class="editor-grid"><article class="typed-form"><div v-for="field in fields" :key="field.key" class="field"><label>{{ field.label }}<small v-if="field.required">必填</small></label><textarea v-model="values[field.key]" :rows="field.kind === 'list' ? 3 : 2" :aria-label="field.label"></textarea></div><button class="button accent" :disabled="saving" @click="save">{{ saving ? '保存中…' : '保存修订' }}</button></article><aside class="evidence-card"><p class="eyebrow">证据</p><h2>证据与来源</h2><div v-if="!evidence.length" class="editor-state small">暂无可展示证据。</div><div v-for="item in evidence" :key="item.evidence_id || item.id" class="evidence-row"><strong>{{ item.source_title || '来源' }}</strong><p><mark>{{ item.quote || item.excerpt || item.content || '无摘录' }}</mark></p><small>{{ item.validation_status || '待验证' }} · {{ item.evidence_strength || '普通证据' }}</small></div></aside></div><article class="revision-card"><div class="revision-heading"><div><p class="eyebrow">修订记录</p><h2>当前版本与历史快照</h2></div><button class="button secondary" :disabled="!selectedRevision" @click="applyRevisionSnapshot">载入所选快照</button></div><div v-if="!revisions.length" class="editor-state small">暂无修订历史。</div><div v-else class="revision-list"><label v-for="item in revisions" :key="item.revision_id" class="revision-row"><input v-model="selectedRevisionId" type="radio" name="knowledge-revision" :value="String(item.revision_id)" /><span><strong>修订 #{{ item.revision_no }}</strong><small>{{ item.change_type || '更新' }} · {{ item.reason || '无说明' }} · {{ item.created_at || '' }}</small></span></label></div></article></template><p v-if="message" class="editor-message">{{ message }}</p></section>
</template>

<style scoped>
.knowledge-editor-page { max-width: 1080px; margin: 0 auto; padding: 12px 0 60px; }.knowledge-editor-page h1 { margin: 10px 0 28px; font-family: Georgia, serif; font-size: clamp(36px, 5vw, 60px); font-weight: 400; letter-spacing: -.055em; }.knowledge-editor-page h1 em { color: var(--accent); font-style: normal; }.override-notice { margin: -14px 0 18px; padding: 10px 12px; border: 1px solid rgba(224,161,125,.3); border-radius: 8px; color: #e4b17c; background: rgba(224,161,125,.08); font-size: 12px; line-height: 1.6; }.editor-grid { display: grid; grid-template-columns: minmax(0, 1.2fr) minmax(260px, .8fr); gap: 16px; }.typed-form, .evidence-card { padding: 22px; border: 1px solid var(--line); border-radius: 16px; background: rgba(255,255,255,.05); }.field { display: grid; gap: 6px; margin-bottom: 15px; }.field label { color: var(--ink); font-size: 12px; }.field label small { margin-left: 7px; color: var(--accent); font-size: 10px; }.field textarea { width: 100%; padding: 10px; resize: vertical; border: 1px solid var(--line); border-radius: 8px; color: inherit; background: transparent; font: 12px/1.6 inherit; }.evidence-card h2 { margin: 5px 0 15px; font-family: Georgia, serif; font-size: 24px; font-weight: 400; }.evidence-row { padding: 10px 0; border-top: 1px solid var(--line); }.evidence-row strong { display: block; font-size: 11px; }.evidence-row p { margin: 5px 0 0; color: var(--muted); font-size: 11px; line-height: 1.6; }.editor-state { min-height: 200px; display: grid; place-items: center; color: var(--muted); font-size: 13px; }.editor-state.small { min-height: 80px; }.editor-message { margin-top: 12px; color: #7da477; font-size: 12px; }

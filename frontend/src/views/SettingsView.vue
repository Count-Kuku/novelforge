...
<template>
  <article v-if="usage" class="usage-panel"><div><p class="eyebrow">USAGE</p><strong>今日 {{ usage.today.request_count || 0 }} 次请求 · {{ usage.today.total_tokens || 0 }} tokens</strong><small>本月 {{ usage.month.request_count || 0 }} 次 · 约 ${{ Number(usage.month.cost_usd || 0).toFixed(4) }}；{{ usage.month.cost_complete ? '价格完整' : '含估算或未计价调用' }}</small></div><span class="usage-cost">今日约 ${{ Number(usage.today.cost_usd || 0).toFixed(4) }}</span></article>
  <article v-if="usage" class="usage-breakdown"><div class="usage-breakdown-heading"><div><p class="eyebrow">USAGE INSPECTOR</p><h2>按维度查看用量</h2></div><select v-model="breakdownDimension" aria-label="用量维度" @change="loadBreakdown"><option value="operation">操作</option><option value="model">模型</option><option value="story">故事</option><option value="project">项目</option><option value="agent">Agent</option></select></div><div v-if="breakdownLoading" class="breakdown-state">正在读取明细…</div><div v-else-if="!breakdownRows.length" class="breakdown-state">当前维度暂无记录。</div><div v-else class="breakdown-table" role="table" aria-label="用量明细"><div class="breakdown-row header" role="row"><span>维度</span><span>请求 / tokens</span><span>费用</span></div><div v-for="row in breakdownRows" :key="String(row.bucket)" class="breakdown-row" role="row"><strong>{{ row.bucket || '未标记' }}</strong><span>{{ row.request_count || 0 }} 次 · {{ row.total_tokens || 0 }} tokens</span><span>${{ Number(row.cost_usd || 0).toFixed(4) }}</span></div></div></article>
  <section class="settings-page"><p class="eyebrow">SETTINGS & CAPABILITIES</p><h1>让工作台知道，<em>哪些能力已经就绪。</em></h1><p class="intro">模型、检索、搜索与 OCR 的状态展示在这里；密钥只接受写入，不会回显到 Vue 页面或 API 响应。</p><div v-if="loading" class="settings-state">正在检查本地能力…</div><template v-else><div class="capability-grid"><article v-for="(item, key) in capabilities" :key="key" class="capability-card"><div class="status-dot" :class="{ ready: item.available }"></div><div><strong>{{ key }}</strong><small>{{ item.provider || '本地 provider' }}</small><p>{{ item.available ? '可用于对应工作流。' : (item.message || item.status) }}</p></div></article></div><article v-if="profiles.length" class="model-panel"><div class="model-panel-heading"><div><p class="eyebrow">MODEL PROFILES</p><h2>模型与凭据</h2></div><span v-if="profileMessage" class="saved-state">{{ profileMessage }}</span></div><div class="profile-tabs"><button v-for="profile in profiles" :key="profile.id" :class="{ active: selectedProfileId === profile.id }" @click="selectProfile(profile)">{{ profile.name || profile.id }}<small>{{ activeProfileId === profile.id ? '当前' : '可切换' }}</small></button></div><div class="model-form"><label>Provider 类型<input v-model="providerType" /></label><label>Base URL<input v-model="baseUrl" placeholder="https://…" /></label><label>模型名称<input v-model="modelName" /></label><label>Embedding 模式<select v-model="embeddingMode"><option value="disabled">关闭</option><option value="openai_compatible">兼容 API</option></select></label><label>Embedding 模型<input v-model="embeddingModelName" /></label><label>API Key（留空保持不变）<input v-model="apiKey" type="password" autocomplete="new-password" placeholder="不会回显已存密钥" /></label></div><div class="model-actions"><button class="button secondary" :disabled="savingProfile" @click="saveProfile">{{ savingProfile ? '保存中…' : '保存模型配置' }}</button><button class="button accent" :disabled="selectedProfileId === activeProfileId" @click="activateProfile">设为活动模型</button></div></article></template><div class="settings-note"><strong>当前故事</strong><span>{{ workspace.activeStory?.name || '未选择故事' }} · {{ workspace.mode === 'conversational' ? '对话模式' : '规划模式' }}</span></div><p v-if="error" class="settings-error">{{ error }}</p></section>
</template>

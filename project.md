...
- 自由创作内置资料托盘，可直接导入文件或粘贴长文；资料默认归属当前故事，也可明确保存到项目，原文用于来源查证并由后台自动提炼知识。
- Vue 两套工作台的资料入口统一为“粘贴资料 / 导入文件”：资料库固定保存项目资料，会话资料默认保存到当前故事并可明确选择项目；保存后自动提炼知识，原文只作为来源查证依据。普通工作台隐藏网络研究任务、来源激活、结论审核和 URL 资料入口；对话消息仍保留联网搜索开关。
...
- Vue 项目批量导入可显式执行本地 Tesseract OCR，并保存逐页置信度；会话资料托盘保留文件格式限制，普通文本层导入不会静默触发 OCR。
- 网络研究后端只支持公开静态文本页面；不执行 JavaScript、不登录网站、不绕过付费墙。LangGraph 只负责单次搜索步骤内的并行分派，SQLite 始终负责持久恢复。该能力不作为普通工作台导航入口。
...
文件或粘贴文本完成附加后先同步词法索引，Embedding 不阻塞开始写作；
...
知识存储采用「实体-事实-关系」时序模型（自 schema 17 引入，当前 schema 23；迁移历史与表结构见 [storage_architecture.md](./storage_architecture.md)）：角色/势力/地点/道具/事件等实体有独立主档（`entities`）；每条知识事实归属实体并带 `fact_key` 槽位键与 `valid_from/to_chapter` 生效区间。片段事实还按正文祖先链取代，同章、无章号和迟到提炼不能仅按任务完成时间判断先后；历史检查点保留当时有效状态。实体间关系与引用统一收编 `graph_edges`；事件用 `world_t` 排序。`constraints` 分类已取消，规则归位到 `world_rules` 与 `rules` 表。

schema 21–23 增加故事世界线、不可变检查点、资料发布版本、故事资料绑定与分支实体身份。`branch_id` 表示本故事的创作分支，既有 `worldline_id` 继续表示来源世界。项目资料的已确认知识经显式绑定创建故事私有副本；分叉采用固定快照及本线覆盖，正文、配置、知识和章节不读取父线后续修改。新故事默认严格绑定，旧故事在明确确认资料范围前保持标注的兼容模式并禁止分叉。数据库升级前使用 SQLite backup 保存一致备份，备份失败则中止升级。

对话“＋资料”在提交时固定会话世界线，默认导入到当前故事；资料库导入固定项目归属。两者自动提炼，原文用于查证；“用于当前故事”复制已确认知识，不重新提炼。资料版本更新仅提示，自动更新、跨线合并及规划台完整多线编辑仍属后续范围；未支持的规划接口明确拒绝非主线请求。实施与验收契约见 [故事资料副本规划](./docs/story-fork-library-plan.md) 和 [世界线隔离规划](./docs/worldline-branch-isolation-plan.md)。
...
资料库的“导入资料”工作区负责粘贴、文件导入和处理状态；“查找与编辑”默认打开统一搜索，支持查看、编辑和修订恢复；“优先设定”是正式知识的高优先级子集，“待审核”确认后进入正式知识。复杂面板只渲染当前选中的工作区，状态键必须按项目/故事作用域隔离。

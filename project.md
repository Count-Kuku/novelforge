...
- `工作台`：项目总览；项目内容通过首页指标按需打开，不在普通侧栏单列入口。
- `资料`：资料导入、知识库。知识库统一展示全部知识、优先设定和待审核知识。
- `规划`：创作配置，以及按故事配置展开的大纲、分卷、剧情段和细纲。
- `写作`：自由创作、章节写作。章节写作内统一提供快速门禁与综合体检；设定提炼是独立知识更新操作。
- `设置`：模型与费用；生成规则和写作偏好收进页面内的高级入口。

普通侧栏隐藏内容管理、检索诊断、独立章节审阅、生成规则和提示词选项等重复或高级页面。内容管理仍可由首页指标按需打开，生成规则和写作偏好由设置页的高级入口集中管理，写作偏好也可在生成页面就地调整；开发者设置 `NOVELFORGE_DEVELOPER_MODE=1` 后可以恢复完整导航。

资料导入页内部工作区为：概览、导入、处理和管理；知识库是正式知识的统一入口，“优先设定”是其中的高优先级子集，“待审核知识”确认后进入正式知识。复杂面板只渲染当前选中的工作区，状态键必须按项目/故事作用域隔离。

...
def _resource_browser_focus_key(project_name: str, story_id: str) -> str:
    return scoped_widget_key("resource_browser_focus", project_name, story_id)


def resource_browser_filter_key(name: str, project_name: str, story_id: str) -> str:
    return scoped_widget_key(name, project_name, story_id)


def _resource_browser_selection_key(project_name: str, story_id: str) -> str:
    return scoped_widget_key("resource_browser_selection", project_name, story_id)
...
def _consume_resource_browser_focus(project_name: str, story_id: str, browser_items: list[dict]) -> tuple[dict, str]:
    focus = st.session_state.pop(_resource_browser_focus_key(project_name, story_id), None)
    ...
    st.session_state[resource_browser_filter_key("resource_browser_search", project_name, story_id)] = focus_search
    st.session_state[resource_browser_filter_key("resource_browser_volume_filter", project_name, story_id)] = 0
    st.session_state[resource_browser_filter_key("resource_browser_arc_filter", project_name, story_id)] = 0
    if focus_groups:
        st.session_state[resource_browser_filter_key("resource_browser_group_filter", project_name, story_id)] = focus_groups
...
def _set_resource_browser_selection(project_name: str, story_id: str, resource: dict):
    st.session_state[_resource_browser_selection_key(project_name, story_id)] = resource


def _get_resource_browser_selection(project_name: str, story_id: str) -> dict:
    return dict(st.session_state.get(_resource_browser_selection_key(project_name, story_id), {}))

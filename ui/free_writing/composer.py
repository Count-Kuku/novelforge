...
_COMPOSER_SHORTCUT = st.components.v2.component(
    "novelforge_composer_shortcut",
    js="""
    export default function(component) {
      const label = component.data.label;
      const key = "nfCreativeComposerShortcut";
      if (window[key]) document.removeEventListener("keydown", window[key], true);
      const handler = (event) => {
...
def _install_composer_shortcut(button_label: str) -> None:
    _COMPOSER_SHORTCUT(
        key="novelforge_composer_shortcut",
        data={"label": str(button_label)},
        height=0,
    )

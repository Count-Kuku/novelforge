...
| 结构化 JSON | 非权威兼容层 | 不写入；旧 JSON 仅作为旧项目一次性导入来源 |
...
不得创建同名空目录或“幽灵”数据库。数据库曾被标记为不可用时，恢复同样使用既有文件模式，不得绕过
DB-only 失败语义。
...
- `creative_messages`：自由创作中的用户消息、普通助手回复和动作回执，不混入正文片段链。
...
## JSON 兼容策略（2026-09-05 收缩）

JSON 镜像写入层已整体移除（原 `NOVELFORGE_WRITE_JSON_MIRRORS` 兼容开关随之删除），SQLite 是结构化数据的唯一权威来源：

- LLM profiles、规则、提示词选项、故事配置、知识、运行快照、检索 manifest/vector/eval/feedback 等结构化数据只写 SQLite。
- 如果首次打开的是旧项目且数据库尚未建立，bootstrap 仍会先从已有 JSON/Markdown 一次性导入，再创建数据库事实源；这是旧 JSON 的唯一用途。
- Markdown/TXT 正文等长文本文件资产不受此策略影响，继续按文件保存。
- 历史版本遗留的 JSON 镜像文件不再被维护或清理，也不会被应用读取（数据库存在时）；手工删除是安全的。

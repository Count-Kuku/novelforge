...
- `工作台`：项目总览；大纲、正文和报告等内容可从项目指标按需打开，不再单列页面入口。
- `资料`：资料导入、知识库。知识库内统一提供全部知识、优先设定和待审核知识视图。
- `规划`：创作配置，以及当前故事需要的大纲、分卷、剧情段和细纲。
- `写作`：自由创作、章节写作；快速和综合审阅已合并到章节写作流程。
- `设置`：模型与费用；生成规则和写作偏好收在页面底部的高级入口。

普通侧栏不再展示内容管理、资料检索诊断、独立章节审阅、生成规则和提示词选项等重复或高级入口。相关底层能力仍保留：内容管理由项目指标按需打开，生成规则和写作偏好由设置页的高级入口集中管理，写作偏好也能在生成页面就地调整；设置 `NOVELFORGE_DEVELOPER_MODE=1` 可显示完整开发者导航。

`资料导入` 负责导入、处理和来源管理；正式知识统一在 `知识库` 查看，其中“优先设定”是需要优先影响创作的知识子集，“待审核知识”确认后才进入正式知识。一般情况下从工作台给出的“推荐下一步”继续即可，不需要理解内部表结构。
...
5. 任务结束后到 `知识库 → 待审核知识` 处理候选。检索健康度、固定评测和索引维护仅在开发者模式下使用。

...
原规划中的 `v0.5.2` 与 `v0.6.0` 已完成。结合后续“用户不断对话生成片段并逐步添加设定”的需求，`v0.7.0` 不再先做孤立的章节候选页，而是把原“快速生成”升级为与知识库、章节体系接壤的“自由创作”工作台。

### 已落地范围

- 保留单提示词生成：新会话的第一轮与原快速生成一样轻量。
- 持久化会话状态：新增 `creative_sessions`、`creative_turns`、`creative_fragments`，SQLite Schema 升级到 `v7`。
- 片段状态机：候选片段必须先接受；支持续写、重写当前候选、从当前创作前沿建立并切换同级候选分支；接受一个候选后，其它同级未接受版本自动转为历史。更早节点的世界线分叉继续使用故事复制，避免已确认旧分支知识污染当前故事。
- 统一上下文复用：每轮调用现有 Context Assembly，注入故事配置、规则、导演注、核心设定、角色/设定实体卡、检索知识、会话摘要和当前分支片段。
- 上下文可审计：片段保存实际 Context Assembly 快照和检索命中；单次导演注仍只在片段成功持久化后消费。
- 设定闭环：只有已接受片段可以提炼结构化候选；候选默认故事级、按需检索并保留来源片段 ID，经用户确认后进入正式知识库，后续生成立即可复用。
- 章节闭环：当前分支的已接受片段可以直接汇编为新章节；已有章节默认禁止覆盖，只有显式追加才会写入。
- 生命周期兼容：故事复制会复制自由创作会话及版本链，故事删除会清理所属会话；项目资源提供只读审计入口。
- 解耦边界：会话只依赖统一上下文和公开知识工作流接口，不直接依赖词法、向量或未来设定合并算法；后续优化提炼策略和向量库无需迁移会话数据。
...
用户提示
  -> 自由创作轮次
  -> 候选片段 proposed
  -> 接受 accepted / 重写 superseded / 分支
  -> 可选提炼为 pending knowledge
  -> 用户确认后进入正式知识库与检索
  -> 后续轮次复用
  -> 当前分支汇编章节并 finalized
```
...
1. 未接受片段不进入长期知识。
2. 重写后的旧候选不参与当前分支上下文和章节汇编。
3. 提炼与确认分离，模型不能自行把猜测写入正式知识。
4. 写作会话保存的是稳定业务状态，不保存特定向量实现细节。
5. 章节仍是正式资产，自由创作是更便捷的形成与筛选过程。

### 验收与后续

`tools/verify_interactive_writing.py` 覆盖会话持久化、世界观复用、版本链、接受门槛、提炼确认、知识回流、章节汇编、复制删除、失败审计和导演注消费。原规划中的人物档案人工覆盖、Character Card 互操作、世界设定高级激活和故事谱系仍作为后续独立批次；它们可复用当前会话与上下文底座，不阻塞本次功能。
...
5. `v0.7.0`：把快速生成升级为自由创作会话、片段候选、知识闭环和章节汇编（已完成）。
6. `v0.8.0+`：在片段候选底座上增加章节级候选稿、选稿发布、故事分支谱系、人物档案与角色卡互操作。
...
- `storage/migrations/008_story_lineage.sql`（实际编号以届时最新迁移为准）
...
| `storage/migrations/008_story_lineage.sql` | 阶段 4 新增 | 故事谱系持久化（实际编号以届时最新迁移为准） |
...
阶段 4 的故事谱系需要可查询的父子关系。由于 Schema v7 已用于自由创作会话，后续应使用新的 Schema v8+ 专表，而不是把关系埋入 `story_profiles` JSON。
